...
def main(
    ...
    Unlike earlier drafts, this version does **not** attempt to guess any
    file paths internally. All input and output paths must be provided
    explicitly by the caller (e.g. from ``sim_test`` using NEBULA_PATH_CONFIG
    or other configuration logic).
    ...
    # Enforce explicit, non-empty upstream paths so we do not rely on
    # NEBULA_PATH_CONFIG or any internal defaults here.
    if not star_projection_path:
        raise ValueError(
            "NEBULA_STAR_SLEW_PROJECTION.main: 'star_projection_path' must be "
            "a non-empty string pointing to obs_star_projections.pkl."
        )
    if not obs_tracks_path:
        raise ValueError(
            "NEBULA_STAR_SLEW_PROJECTION.main: 'obs_tracks_path' must be "
            "a non-empty string pointing to observer_tracks_with_pointing.pkl "
            "(or observer_tracks_with_pixels.pkl)."
        )
